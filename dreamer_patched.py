...
class Dreamer(nn.Module):
    ...
    def _cal_grad(self, data, initial):
        ...
        if self.rep_loss == "dreamer":
            ...
            # ADARR: log cross-correlation diagnostics for H_off analysis
            mets["hoff_frobenius"] = redundancy_loss.item()  # already computed as sum of squares
            with torch.no_grad():
                c_off_abs = c[off_diag_mask].abs()
                c_sum = c_off_abs.sum()
                if c_sum > 1e-10:
                    p = c_off_abs / c_sum
                    p = p[p > 1e-12]
                    mets["hoff_entropy"] = -torch.sum(p * torch.log(p)).item()
                else:
                    mets["hoff_entropy"] = 0.0
                mets["hoff_diag_err"] = (torch.diagonal(c) - 1.0).abs().mean().item()
                mets["hoff_offdiag_mean"] = c_off_abs.mean().item()
